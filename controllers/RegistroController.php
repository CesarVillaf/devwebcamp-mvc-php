...
class RegistroController {
    ...
    public static function crear(Router $router) {
        ...
        if(isset($registro) and ($registro->paquete_id === '3' or $registro->paquete_id === '2')) {
            header('Location: /boleto?id=' . urlencode($registro->token));
            return;
        }

        if(isset($registro) and $registro->paquete_id === '1') {
    ...
    public static function gratis() {
        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST') {
            if(!is_auth()) {
             ...
             // Verificar si el usuario ya esta registrado:
            $registro = Registro::where('usuario_id', $_SESSION['id']);
            if(isset($registro) and $registro->paquete_id === '3') {
                header('Location: /boleto?id=' . urlencode($registro->token));
                return;
            }
    ...
    public static function conferencias(Router $router) {
        ...
        if(isset($registro) and $registro->paquete_id === '2') {
            header('Location: /boleto?id=' . urlencode($registro->token));
            return;
        }
        ...
        if(isset($registro->regalo_id) and $registro->paquete_id === '1') {
            header('Location: /boleto?id=' . urlencode($registro->token));
            return;
        }
        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if(!isset($registro) or $registro->paquete_id !== '1') {
                echo json_encode(['resultado' => false]);
                return;
            }
            ...
            foreach($eventos as $evento_id) {
                ...
                // Comprobar que el evento exita:
                if(!isset($evento) or $evento->disponibles === '0') {
                    echo json_encode(['resultado' => false]);
                    return;
                }
